...
public class SocketServer extends Thread {
    ...
    private void checkLogin(Socket server) {
        ...
        int count = 0;
        boolean success = checkLoginOneTime();
        while (!success) {      //检查登录信息
            ...
            count++;
            if (count == 10) {  //修bug
                logAppend(consoleLog, "用户名或密码多次错误 中断连接");
                exit();
                return;
            }
            success = checkLoginOneTime();
        }
        logAppend(consoleLog, "用户登录成功");
        try {
    ...
    private void receiveMessage() {      //服务端接收信息
        ...
        try {
            ...
            logAppend(consoleLog, "与当前客户端断开连接!");
        }
    }
